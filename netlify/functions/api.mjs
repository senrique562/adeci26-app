...
const PUNTOS = { checkin: 60, triviaCorrecta: 30, triviaBonus: 30, eposter: 90, comentario: 30, comentariosMax: 5, visita: 150 };
...
// ───────────────────────── progreso ─────────────────────────
function progreso(u) {
  const m1 = u.m1 || {}, m2 = u.m2 || {}, m4 = Array.isArray(u.m4) ? u.m4 : [];
  let gotas = 0;
  const dias1 = Object.keys(m1).length;
  gotas += dias1 * PUNTOS.checkin;
  let triviaDias = 0;
  for (const d of ['1', '2']) {
    if (m2[d]) {
      triviaDias++;
      gotas += (m2[d].correctas || 0) * PUNTOS.triviaCorrecta;
      if ((m2[d].correctas || 0) === (m2[d].total || 3)) gotas += PUNTOS.triviaBonus;
    }
  }
  if (u.m3) gotas += PUNTOS.eposter;
  gotas += Math.min(m4.length, PUNTOS.comentariosMax) * PUNTOS.comentario;
  if (u.m5) gotas += PUNTOS.visita;
...
const acciones = {
  ...
  async register({ body, fs }) {
    ...
    if (idx.exists) {
      ...
      uid = newUid();
      u = { uid, nombre, apellido, email, creado: Date.now(), m1: {}, m2: {}, m3: null, m4: [], m5: null };
  ...
  async checkin({ auth, body, fs }) {
    const dia = Number(body.dia);
    if (!DIAS[dia]) return err('Día inválido.');
    if (!diaActivo(dia)) return err('El check-in de este día se habilita el ' + DIAS[dia].split('-').reverse().join('/') + '.');
    const ref = fs.collection('users').doc(auth.u);
    await ref.set({ m1: { [dia]: Date.now() } }, { merge: true });
    const u = await getUser(fs, auth.u);
  ...
  async trivia_get({ auth, body, fs }) {
    ...
    const preguntas = (t?.preguntas || []).map(({ correcta, explicacion, ...q }) => q);
  ...
  async trivia_answer({ auth, body, fs }) {
    ...
    const preguntas = tSnap.exists ? (tSnap.data().preguntas || []) : [];
    ...
    const detalle = preguntas.map((q, i) => {
      ...
      if (ok) correctas++;
      return { elegida: resp[i], correcta: Number(q.correcta), ok, explicacion: q.explicacion || '' };
    });
  ...
  async eposter({ auth, body, fs }) {
    const titulo = clean(body.titulo, 220);
    if (titulo.length < 4) return err('Escribí el título del trabajo (al menos 4 caracteres).');
    await fs.collection('users').doc(auth.u).set({ m3: { titulo, at: Date.now() } }, { merge: true });
    const u = await getUser(fs, auth.u);
    return json({ ok: true, progreso: progreso(u) });
  },
  ...
  async staff_scan({ auth, body, fs }) {
    ...
    const uid = clean(body.uid, 20).toUpperCase().replace(/^ADECI26:/, '');
    ...
    const yaEstaba = !!u.m5;
    if (!yaEstaba) {
      await fs.collection('users').doc(uid).set({ m5: { at: Date.now(), por: auth.e } }, { merge: true });
      await fs.collection('stats').doc('stand').set({ visitas: FieldValue.increment(1) }, { merge: true });
    }
  ...
  async staff_stats({ auth, fs }) {
    ...
    return json({ visitas: s.exists ? (s.data().visitas || 0) : 0 });
  },
  ...
  async admin_trivia_save({ auth, body, fs }) {
    ...
    const dia = Number(body.dia);
    if (!DIAS[dia]) return err('Día inválido.');
    const preguntas = (Array.isArray(body.preguntas) ? body.preguntas : []).slice(0, 10).map((q, i) => ({
      id: 'q' + (i + 1),
  ...
  async admin_metricas({ auth, fs }) {
    ...
    const M = { total: 0, m1: 0, m2: 0, m3: 0, m4: 0, m5: 0, completos: 0, checkinD1: 0, checkinD2: 0, triviaD1: 0, triviaD2: 0, comentarios: 0, gotasTotal: 0, porCompletos: [0, 0, 0, 0, 0, 0] };
    const comentariosPorSesion = {};
    snap.forEach(d => {
      ...
      if (u.m1?.[1]) M.checkinD1++; if (u.m1?.[2]) M.checkinD2++;
      if (u.m2?.[1]) M.triviaD1++; if (u.m2?.[2]) M.triviaD2++;
    ...
    return json({ ...M, comentariosPorSesion });
  },
  ...
  async admin_export({ auth, body, fs }) {
    ...
    if (tipo === 'comentarios') {
      ...
      filas.push(['nombre', 'apellido', 'email', 'titulo_eposter', 'fecha'].map(esc).join(';'));
      snap.forEach(d => { const u = d.data(); if (u.m3) filas.push([u.nombre, u.apellido, u.email, u.m3.titulo, new Date(u.m3.at).toISOString()].map(esc).join(';')); });
    } else {
      filas.push(['uid', 'nombre', 'apellido', 'email', 'registro', 'm1', 'm2', 'm3', 'm4', 'm5', 'completos', 'gotas', 'visita_stand', 'trivia_d1', 'trivia_d2'].map(esc).join(';'));
      snap.forEach(d => {
        const u = d.data(); const p = progreso(u);
        filas.push([u.uid, u.nombre, u.apellido, u.email, new Date(u.creado || 0).toISOString(),
          ...
          p.completos, p.gotas, u.m5 ? new Date(u.m5.at).toISOString() : '',
  ...
  async admin_sorteo({ auth, body, fs }) {
    if (auth.r !== 'admin') return err('No autorizado.', 403);
    const modo = body.modo === 'visitas' ? 'visitas' : 'completos';
    const excluir = new Set((Array.isArray(body.excluir) ? body.excluir : []).map(String));
    const snap = await fs.collection('users').get();
    const candidatos = [];
    snap.forEach(d => {
      const u = d.data(); const p = progreso(u);
      const elegible = modo === 'visitas' ? !!u.m5 : p.habilitado;
