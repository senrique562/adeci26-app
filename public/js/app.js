...
import { DIAS, EJES, PROGRAMA } from '/data/programa.js';
import { speaker } from '/data/speakers.js';
import { AUTORIDADES, COMITE_CIENTIFICO, EJES_TEMATICOS } from '/data/comite.js';
...
const ejeTag = (s) => s.eje && EJES[s.eje] ? `<span class="eje ${EJES[s.eje].clase}">${esc(EJES[s.eje].nombre)}</span>` : '';
...
function vAhora() {
  ...
  const faltan = 5 - completos;
  return appbar('ADECI<span>26</span>', null, `<span class="muted" style="color:var(--celeste)">${esc(S.user?.nombre || '')}</span>`) + `<div class="body">
    ${avisos}
    ${live}
    ${prox}
    <div class="sec-t">Tus 5 momentos</div>
    <div class="card tap progress-card" data-go="juego">
      <div class="ic">💧</div>
      <div class="grow"><div class="b" style="font-size:14.5px">${completos} de 5 momentos · ${p?.gotas ?? 0} gotas</div>
      <div class="muted">${completos === 5 ? '¡Completaste la ruta! Ya participás del sorteo.' : `Te falta${faltan === 1 ? '' : 'n'} ${faltan} para el sorteo de ADOX`}</div></div>
      <div class="chev">›</div>
    </div>
    <div class="sec-t">Con el apoyo de</div>
    <div class="card tap center" data-go="adox" style="padding:18px 15px">
      <img src="/img/adox-wordmark.png" alt="ADOX" style="height:34px">
      <div class="muted mt-s">${esc(S.config.standTexto || 'Stand de ADOX')} · Cómo participar del sorteo →</div>
    </div>
    ${hoy ? '' : `<div class="muted center mt">Hoy no es día de congreso. Las acciones del juego se habilitan el 17 y 18 de septiembre.</div>`}
  </div>`;
}
...
function vPrograma() {
  ...
  const lista = programaDia(n).map(s => {
    ...
    return `<div class="card slot ${brk ? 'break' : ''} ${s.tipo === 'acto' ? 'key' : ''} ${live ? 'live-now' : ''} ${brk ? '' : 'tap'}" ${brk ? '' : `data-go="sesion/${s.id}"`}>
      ...
      ${ejeTag(s) ? `<div style="margin-top:6px">${ejeTag(s)}</div>` : ''}
  ...
  return appbar('Programa') + `<div class="days">${DIAS.map(d => `<button class="${d.n === n ? 'on' : ''}" data-dia="${d.n}">${d.label} · Día ${d.n}</button>`).join('')}</div>
    ...
    <div class="muted center mt">Tocá una sesión para ver quién participa y dejar tu comentario.<br>Sala plenaria única.</div></div>`;
}
...
function vSesion(id) {
  ...
  return appbar('Sesión', 'programa') + `<div class="hero">
      ...
      <h1>${esc(s.titulo)}</h1><p>Sala plenaria</p>
    </div>
    <div class="body">
      ${ejeTag(s) ? `<div class="mb">${ejeTag(s)}</div>` : ''}
      ${gente}
      ${comentable ? `<div class="sec-t" id="comentar">Tu comentario sobre esta sesión</div>
      <div class="card">
        <div class="muted small" style="margin-bottom:10px">Qué te llevás, qué te sorprendió, qué aplicarías en tu servicio. Comentar tres sesiones completa el <b>momento 4</b>.</div>
...
function vJuego() {
  const p = S.progreso || { momentos: {}, completos: 0, gotas: 0, detalle: { checkins: [], trivia: {}, comentarios: [] } };
  ...
  // M1
  let m1act = '';
  if (hoy && !ck.includes(hoy)) m1act = `<div class="act"><button class="btn btn-p sm" data-act="checkin" data-dia="${hoy}">Confirmar que llegué · Día ${hoy}</button></div>`;
  else if (!hoy) m1act = `<div class="st" style="color:var(--txt-3)">Se habilita el 17 de septiembre, cuando llegues al congreso.</div>`;
  const m1st = ck.length ? `<div class="st">Check-in ${ck.map(x => 'día ' + x).join(' y ')} ✓ · +${ck.length * 60} gotas</div>` : '';

  // M2
  const tr = d.trivia || {};
  const trDias = [1, 2].filter(x => tr[x]);
  let m2st = trDias.map(x => `Día ${x}: ${tr[x].correctas}/${tr[x].total} correctas`).join(' · ');
  let m2act = '';
  ...
  else if (!hoy) m2act = `<div class="st" style="color:var(--txt-3)">Cada día del congreso, 3 preguntas nuevas.</div>`;

  // M3
  const m3 = d.eposter;
  const m3act = m3 ? `<div class="st">Anotaste: «${esc(m3)}» · +90 gotas</div><div class="act"><button class="btn link" data-act="eposter-edit">Cambiar</button></div>`
    : `<form class="act" id="f-eposter"><div class="field" style="margin-bottom:8px"><input name="titulo" maxlength="220" placeholder="Título del trabajo que visitaste" required></div><button class="btn btn-p sm" type="submit">Guardar</button></form>`;

  // M4
  const nc = (d.comentarios || []).length;
  const m4st = `<div class="st" style="${nc >= 3 ? '' : 'color:var(--txt-3)'}">${Math.min(nc, 3)} de 3 comentarios${nc ? ` · +${Math.min(nc, 5) * 30} gotas` : ''}</div>`;
  ...
  // M5
  const m5st = m.m5 ? `<div class="st">Visitaste el stand ✓ · +150 gotas</div>` : '';
  const m5act = m.m5 ? '' : `<div class="act"><button class="btn btn-p sm" data-go="yo">Mostrar mi credencial</button></div>`;

  return appbar('Tus 5 momentos') + `<div class="body">
    <div class="score ${p.completos === 5 ? 'done' : ''}">
      <div><div class="lb">Tus gotas</div><div class="big">${p.gotas}</div></div>
      <div class="rt"><b>${p.completos} de 5</b>momentos completos</div>
    </div>
    ...
    <div class="card" style="border-left:3px solid var(--teal);margin-bottom:16px">
      ...
      <div style="font-size:24px;font-weight:900;color:var(--navy);letter-spacing:-.6px;line-height:1.1;margin-top:5px">¡Sumá gotas y ganá!</div>
      <div class="small mt-s" style="color:var(--txt-2);line-height:1.5">Durante el congreso vas a pasar por los cinco momentos de siempre, los de la higiene de manos de la OMS. Solo que esta vez cada momento se cumple haciendo algo del congreso: escuchar, recorrer los e-pósters, comentar y pasar por el stand. Cada momento suma gotas.</div>
      <div style="margin-top:14px;padding-top:13px;border-top:1px solid var(--line)">
        <div class="muted" style="font-size:11px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase;margin-bottom:9px">Cómo entrás al sorteo</div>
        <div class="stepline"><span class="numb">1</span><div>Completá los <b class="b">5 momentos</b>. El quinto es la visita al stand de ADOX: ahí te escanean la credencial de la app.</div></div>
        <div class="stepline"><span class="numb t">2</span><div>Con los cinco completos, tu credencial aparece <b class="b">habilitada</b> y entrás en el sorteo.</div></div>
      </div>
      <div class="callout mt-s"><div class="t">El sorteo es en el stand de ADOX</div><div class="x">${esc(S.config.sorteoInfo || 'Hay que estar presente: si sale tu nombre y no estás, se sortea de nuevo.')}</div></div>
      <div class="muted mt" style="font-size:11.5px">Basado en «Mis 5 momentos para la higiene de manos», OMS © 2009.</div>
    </div>
    ...
    <div class="ruta">
      ...
      <div class="${stepCls('m1', primeroPendiente === 'm1')}"><div class="num">${m.m1 ? '✓' : '1'}</div>
        <div class="mm">Momento 1</div><div class="tt">Antes del contacto con el paciente</div>
        <div class="dd"><b class="b">En el congreso:</b> registrate y confirmá que llegaste, cada día.</div>${m1st}${m1act}</div>
      <div class="${stepCls('m2', primeroPendiente === 'm2')}"><div class="num">${m.m2 ? '✓' : '2'}</div>
        <div class="mm">Momento 2</div><div class="tt">Antes de realizar una tarea limpia o aséptica</div>
        <div class="dd"><b class="b">En el congreso:</b> respondé la trivia del día. Tres preguntas de control de infecciones.</div>${m2st ? `<div class="st">${m2st}</div>` : ''}${m2act}</div>
      <div class="${stepCls('m3', primeroPendiente === 'm3')}"><div class="num">${m.m3 ? '✓' : '3'}</div>
        <div class="mm">Momento 3</div><div class="tt">Después del riesgo de exposición a líquidos corporales</div>
        <div class="dd"><b class="b">En el congreso:</b> recorré los e-pósters y contanos el título de uno de los trabajos que visitaste.</div>${m3act}</div>
      <div class="${stepCls('m4', primeroPendiente === 'm4')}"><div class="num">${m.m4 ? '✓' : '4'}</div>
        <div class="mm">Momento 4</div><div class="tt">Después del contacto con el paciente</div>
        <div class="dd"><b class="b">En el congreso:</b> dejá un comentario sobre tres sesiones en las que participaste.</div>${m4st}${m4act}</div>
      <div class="${stepCls('m5', primeroPendiente === 'm5')}" style="min-height:auto"><div class="num">${m.m5 ? '✓' : '5'}</div>
        <div class="mm">Momento 5</div><div class="tt">Después del contacto con el entorno del paciente</div>
        <div class="dd"><b class="b">En el congreso:</b> pasá por el stand de ADOX y mostrá tu credencial para que la escaneen.</div>${m5st}${m5act}</div>
    </div>
  </div>`;
}
...
function vTrivia(dia, data) {
  ...
  if (!data.habilitado) cuerpo = `<div class="empty">La trivia del ${dl.label.toLowerCase()} se habilita ese día.</div>`;
    ...
    cuerpo = `<div class="score ${r.correctas === r.total ? 'done' : ''}"><div><div class="lb">Tu resultado</div><div class="big">${r.correctas}/${r.total}</div></div><div class="rt"><b>+${r.correctas * 30 + (r.correctas === r.total ? 30 : 0)} gotas</b>${r.correctas === r.total ? 'incluye bonus por todas correctas' : 'cada correcta suma 30'}</div></div>` +
...
function vAdox() {
  const m5 = S.progreso?.momentos?.m5;
  ...
  <div class="body">
    <div class="card" style="border-left:3px solid var(--adox)">
      ...
      <div class="small mt-s" style="color:var(--txt-2)">Acá se juega el quinto momento y acá se hace el sorteo del congreso.</div>
    </div>
    <div class="sec-t">Cómo participar</div>
    <div class="card">
      <div class="stepline"><span class="numb">1</span><div><b class="b">Completá los momentos 1 a 4</b> en la app: check-in, trivia, e-pósters y tres comentarios.</div></div>
      <div class="stepline"><span class="numb">2</span><div><b class="b">Vení al stand con tu credencial de la app.</b> Te la escaneamos y sumás el momento 5 (+150 gotas).</div></div>
      <div class="stepline" style="margin-bottom:0"><span class="numb t">3</span><div><b class="b">Quedate para el sorteo.</b> ${esc(S.config.sorteoInfo || 'Se sortea entre quienes completaron los 5 momentos. Hay que estar presente.')}</div></div>
      <button class="btn ${m5 ? 'btn-g' : 'btn-adox'} full mt" data-go="${m5 ? 'juego' : 'yo'}">${m5 ? 'Ver mis 5 momentos' : 'Mostrar mi credencial'}</button>
    </div>
...
function vYo() {
  ...
  return appbar('Mi credencial') + `<div class="body">
    <div class="badge">
      ...
      <div class="hab ${hab ? 'on' : ''}">${hab ? '✓ Habilitado para el sorteo' : `${p?.completos ?? 0} de 5 momentos`}</div>
    </div>
    <div class="muted center small mb">Mostrá este código en el stand de ADOX para acreditar tu visita.</div>
    <div class="card">
      <div class="kv"><span class="k">Correo</span><span class="v">${esc(u.email)}</span></div>
      <div class="kv"><span class="k">Gotas</span><span class="v" style="color:var(--teal)">${p?.gotas ?? 0}</span></div>
      <div class="kv"><span class="k">Momentos</span><span class="v">${p?.completos ?? 0} de 5</span></div>
    </div>
    ...
    ${S.config.libroUrl ? `<a class="card tap row" href="${esc(S.config.libroUrl)}" target="_blank" rel="noopener" style="display:flex;text-decoration:none;color:inherit"><div class="grow"><div class="b">Libro de resúmenes</div><div class="muted">Abrir PDF ↗</div></div><span style="color:var(--txt-3);font-size:20px">›</span></a>`
    ...
    <div class="card tap row" data-go="ejes"><div class="grow"><div class="b">Ejes temáticos</div><div class="muted">Los ocho ejes del congreso</div></div><span style="color:var(--txt-3);font-size:20px">›</span></div>
    <div class="sec-t">Información útil</div>
    <div class="card">
      <div class="kv"><span class="k">Sede</span><span class="v">Córdoba</span></div>
...
function vEjes() {
  return appbar('Ejes temáticos', 'yo') + `<div class="body"><div class="card">${EJES_TEMATICOS.map((e, i) => `<div class="kv"><span class="k" style="min-width:24px">${i + 1}</span><span class="v" style="text-align:left;flex:1">${esc(e)}</span></div>`).join('')}</div></div>`;
}
...
// ───────────────────────── render + eventos ─────────────────────────
const NAVMAP = { ahora: 'ahora', programa: 'programa', sesion: 'programa', juego: 'juego', trivia: 'juego', adox: 'adox', yo: 'yo', comite: 'yo', ejes: 'yo' };
let triviaCache = {};
...
function dibujarQR() {
  ...
  const draw = () => { el.innerHTML = ''; new QRCode(el, { text: 'ADECI26:' + S.user.uid, width: 168, height: 168, colorDark: '#012951', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.M }); };
...
document.addEventListener('click', async (e) => {
  ...
  if (act) {
    ...
    if (a === 'checkin') { act.disabled = true; try { const d = await api('checkin', { dia: act.dataset.dia }); S.progreso = d.progreso; toast('¡Check-in listo! +60 gotas'); render(); } catch (er) { toast(er.message, true); act.disabled = false; } }
...
document.addEventListener('submit', async (e) => {
  ...
  try {
    if (f.id === 'f-login') {
      ...
      toast(n >= 3 ? '¡Comentario guardado! Momento 4 completo.' : `Comentario guardado (${n} de 3).`); render();
    } else if (f.id === 'f-eposter') {
      const d = await api('eposter', { titulo: fd.get('titulo') }); S.progreso = d.progreso; toast('¡Momento 3 completo! +90 gotas'); render();
      ...
      toast(`${d.resultado.correctas} de ${d.resultado.total} correctas`); cargarTrivia(dia); render();
    }
